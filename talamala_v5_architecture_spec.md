...
> **نسخه:** 2.1 (Revised — جلسه‌ی بازبینیِ عمیقِ 2026-05-16…18)
> **تاریخ:** 2026-05-18
> **وضعیت:** آماده برای داوریِ خارجی، سپس فاز پیاده‌سازی
> **منبع:** سه دور Q&A با تیم Goldis Ops + ادغام ChatGPT + ۳۵ تصمیمِ اصلاحیِ بازبینی (D-46…D-80)

---

## ۰.۰ راهنمای داورِ خارجی (⚠️ قبل از هر بازبینی بخوان)

این سند **بارها بازنویسی شده**. برای جلوگیری از گزارشِ تناقض‌هایِ از‌قبل‌حل‌شده:

1. **منبعِ حقیقت = بخش ۲.۵ (جدولِ Override، D-46…D-80).** هر جای بدنه‌ی سند که با این جدول تعارض داشته باشد، **جدول معتبر است**. قبل از گزارشِ هر تناقض، اول §۲.۵ را چک کن.
2. متنِ `~~خط‌خورده~~` یا بلوک‌های دارای بنرِ «منسوخ/تاریخی» = عمداً نگه داشته شده برای ردگیری؛ **تناقض نیست**.
3. ابهاماتِ فهرستِ بخش ۲۰: همه‌ی `Q-01…Q-10` **حل شده** (✅ RESOLVED). نقاطِ کشف‌شده‌ی A-1…A-13 و F-1…F-4 هم در §۲.۵ حل شده‌اند.
4. **تمرکزِ بازبینی روی بخش ۰.۱ (موارد باز) باشد** — نه بازکردنِ تصمیماتِ قطعی‌شده‌ی §۲.۵ (آن‌ها با تیم نهایی شده و نباید دوباره بحث شوند مگر باگِ مالیِ واقعی پیدا شود).
...
| D-04 | Wallet scope | ~~Per-legal-entity (دو wallet)~~ — **override شد با D-46: سه کیفِ ایزوله per wallet-scope (goldis/aminzar/talamala)** | تیم |
...
| D-29 | AminZar wallet | ~~merge در Goldis~~ — **override شد با D-46: scope=aminzar کاملاً جدا و ایزوله** |
...
| D-32 | **Refund/Buyback model** | **Refund حذف.** ⚠️ **اصلاح‌شده با D-58/D-68: «cancel» وجود ندارد و digital_buyback جدا نیست.** Buyback فقط **۲ حالت**: (a) تحویل‌نشده (بازخریدِ آنلاینِ مستقل، نه cancel — D-58/D-59)، (b) حضوری. «بازخریدِ دیجیتال» = همان `digital_trade sell` (D-68/D-70). در هر دو: وزن خالص طلا به wallet XAU_MG + `buyback_credit_rial` (با شرطِ ثبت‌مالکیت+OTP — D-53) به wallet IRR؛ اجرت/مالیات/سود می‌سوزد. |
...
### تصمیمات جلسه‌ی بازبینی 2026-05-16/17 (Override — D-46…D-80)
