...
| D-39 | Profit share | **بدون profit share در v1**. هر فروشنده، کل سود فروش (اجرت + مالیات + اختلاف قیمت) را خودش نگه می‌دارد. Goldis فقط بابت hedging قیمت طلای خام را می‌گیرد و طلای خام را تحویل می‌دهد. تیم گفت: «بابت فروش کسی سودی به کسی نمی‌دهد» |
...
CREATE TABLE asset_balances (
    ...
    company_id BIGINT NOT NULL REFERENCES companies(id),  -- legal entity owner of wallet
    ...
    PRIMARY KEY (user_id, company_id, asset_type_id),
...
> - **+** : exposure باز (Goldis بدهکار طلا شد) → user_buy (digital)، marketplace_sale، physical_sale در هر brand (شامل فروش‌های خود Goldis هم — چون Goldis باید خام بخرد)
> - **−** : exposure بسته (Goldis طلا را از بازار خرید یا کاربر طلا را برگرداند) → `hedge_buy`، `user_sell` (digital_trade sell)، `buyback`، `physical_purchase_from_wallet`
>
...
CREATE TABLE treasury_settings (
    ...
    max_open_exposure_mg BIGINT NOT NULL,
...
3. raw_hedge_rial = sum(order_items.raw_hedge_price_rial)
   # raw_hedge_price = raw_gold_price_per_mg × pure_gold_mg
   # snapshot قیمت طلای خام در لحظه‌ی فروش، ذخیره در order_item
...
### ۷.۲. ⚠️ Supplier Purchase = خارج از Scope v1

تصمیم تیم: **فرآیند خرید Goldis از کارخانه‌ها (AminZar و TalaMala) داخل سامانه پیاده‌سازی نمی‌شود.** این تسویه‌ها بیرون از پلتفرم (به‌صورت دستی/توافقی) انجام می‌شوند.
...
├── purity SMALLINT NOT NULL             -- 999, 9999
...
CREATE TABLE channel_pricing_formulas (
    ...
    -- D-28: ۲ دقیقه default، مجاز تا حداکثر ۳۰۰s (۵ دقیقه)
    priority INT NOT NULL DEFAULT 0,
    is_active BOOLEAN NOT NULL DEFAULT TRUE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT chk_lock_ttl CHECK (lock_ttl_seconds BETWEEN 30 AND 300)
);
...
CREATE TABLE order_items (
    ...
    cost_price_rial BIGINT NULL,             -- برای settlement
    supplier_price_rial BIGINT NULL,         -- اگر AminZar
...
4. POST /api/v1/checkout/start →
   ...
   d. INSERT order_items با buyback snapshot (D-32) + cost snapshot (D-06b):
      ...
      - cost_price_rial = internal_base_price × pure_gold_mg / 1000
        (cost lock در لحظه‌ی فروش — برای inter_company_ledger بعداً)
   ...
   e. **اگر payment_receiver != Goldis** (یعنی فروش TalaMala-side):
      INSERT inter_company_ledger × 2:
        (debtor=TalaMala, creditor=Goldis, asset=rial,
         amount=sum(order_items.cost_price_rial))
...
- Treasury مرکزی Goldis: +500mg (مهم! treasury مرکزی است حتی اگر
  پول رفته به TalaMala)
- Inter-company ledger × 2 (TalaMala-side):
  • TalaMala → Goldis، rial، cost = internal_base_price × 0.5g
...
6. confirm:
   ...
   - Treasury.record(source=digital_sell, delta=-300mg)
   - Settlement: Goldis طلب از TalaMala (Goldis طلا را پس می‌گیرد)
   - Outbox + Notification
```
...
12. confirm (after gateway verified یا اگر gateway نداشت بلافاصله):
    ...
    - Treasury.record(source=physical_purchase_from_wallet, delta=-gold_part_mg)
      توضیح: gold از wallet کاربر خارج می‌شود و معادلش به فیزیکی تبدیل می‌شود
              ⇒ net change در exposure Goldis منفی است
...
3. اتومات (در یک DB transaction):
   ...
   - Settlement.reverse_item (cost transfer reverse)
   - Audit log
...
#### زیرflow (c) — Digital Buyback (آنلاین، اتومات)
...
| Q-05 | cost transfer در buyback: آیا cost transfer reverse می‌شود (Goldis به TalaMala پس می‌دهد)؟ | Settlement |
| Q-06 | KYC user_level: AntiML thresholds در آینده — الان چه levelهایی فعالند؟ | KYC |
| Q-07 | برای bar که در brand خاصی pre-assign شده، اگر مدت زیادی نفروخته شد، چه می‌شود؟ TTL/auto-reassignment؟ | Inventory |
| Q-08 | Goldis IPG های متعدد (چند PSP) — کدام default؟ کاربر می‌تواند انتخاب کند؟ | Payment |
| Q-09 | کاربر می‌تواند نام شخص دیگر در withdrawal ریال داشته باشد یا فقط حساب خودش؟ | Withdrawal + Compliance |
| Q-10 | برای physical_purchase_from_wallet که cost از wallet خارج می‌شود، خود طلای فیزیکی از کجا تأمین می‌شود؟ از موجودی شرکت یا متناسب با channel؟ | Inventory + Treasury |
