...
- **قابلیت فروش** (`is_sellable`): Boolean، پیش‌فرض `false` — دروازه فروش شمش
  - **موجودی نمایش‌داده‌شده در همه کانال‌ها = تعداد شمش‌های `is_sellable = true`**
  - کانال‌های تحت تأثیر: فروشگاه (لیست + جزئیات + تفکیک نمایندگی)، سبد خرید، چک‌اوت (موجودی نمایندگان و انبار پستی)، تخصیص شمش هنگام ثبت سفارش (ریالی و طلایی)، POS نماینده، POS مشتری، REST API نماینده، سینک Rasis
  - **Opt-in**: شمش تازه‌ساخته‌شده به‌صورت پیش‌فرض قابل فروش **نیست** و باید صراحتاً فعال شود
  - مستقل از `status` عمل می‌کند: شمش می‌تواند `Assigned` باشد ولی قابل فروش نباشد (مثلاً رزرو فیزیکی، ایراد کیفی، نمایشگاه)
  - تغییر این فیلد رزرو فعال (`reserved_customer_id`) را پاک نمی‌کند
  - همگام‌سازی Rasis: فعال‌کردن → `add_bar_to_pos`، غیرفعال‌کردن → `remove_bar_from_pos` (best-effort، هرگز عملیات ادمین را بلاک نمی‌کند)
  - ورودی رسید Rasis عمداً این فیلد را چک نمی‌کند — فروشی که فیزیکاً روی دستگاه انجام شده باید ثبت شود
- ارتباطات: محصول، مالک (user_id FK → users)، بچ‌ها (M2M از طریق `bar_batch_links` — یک شمش می‌تواند از چند ذوب باشد)، بسته‌بندی، نمایندگی (dealer_id FK → users where is_dealer=True)
- تصاویر شمش (BarImage)
...
- **`Reserved` عمداً مجاز نیست**: رزرو همراه با `reserved_customer_id` و `reserved_until` توسط سبد خرید/POS ساخته می‌شود. ست‌کردن گروهی آن شمشی می‌سازد که Reserved است ولی نه دارنده دارد نه انقضا — و `release_expired_reservations()` هرگز آزادش نمی‌کند، یعنی برای همیشه از موجودی خارج می‌شود. برای موارد خاص از فرم ویرایش تکی استفاده کنید
- اعتبارسنجی (`_validate_bulk_status`) بر اساس **وضعیت نهایی هر شمش** انجام می‌شود (ترکیب مقادیر گروهی با مقادیر فعلی هر شمش):
  | وضعیت هدف | شرط لازم |
  |---|---|
  | `Raw` | هیچ شمشی نباید محصول یا مالک داشته باشد (باید «حذف محصول» و «حذف مالک» هم انتخاب شود) |
  | `Assigned` | همه شمش‌ها باید نمایندگی (مکان) و محصول داشته باشند |
  | `Sold` | علاوه بر نمایندگی و محصول، همه باید مالک داشته باشند |
- در صورت نقض، `ValueError` با پیام فارسی شامل **تعداد** شمش‌های مشکل‌دار پرتاب می‌شود و هیچ تغییری اعمال نمی‌گردد
- `is_sellable` مستقل است و با تغییر وضعیت دست‌نخورده می‌ماند
| GET | `/admin/bars/{id}/qr` | تولید و ارسال QR Code با کیفیت بالا (PNG) — تولید لحظه‌ای بدون ذخیره روی دیسک |
...
- **تصویر جعبه‌های کادو** کنار هر آیتم نمایش داده می‌شود؛ کلیک روی تصویر مقدار `select` را ست و رویداد `change` را شلیک می‌کند، پس همان مسیر AJAX موجود (`/api/cart/set-gift-box`) تنها منبع حقیقت می‌ماند
- قیمت جعبه هدیه در مبلغ خط سفارش لحاظ می‌شود
- **ریز محاسبه قیمت هر آیتم** (collapse قابل کلیک): فرمول (وزن × عیار × قیمت هر گرم) + ارزش طلای خام، اجرت (با درصد)، مالیات (با درصد، روی اجرت)، قیمت هر عدد، جعبه کادو، و ضرب در تعداد
- **تفکیک جمع کل** در خلاصه سفارش: مجموع طلای خام، مجموع اجرت، مجموع مالیات، مجموع جعبه کادو، جمع کل
  - `cart_service.summarize_totals(items)` — از روی همان `details` هر آیتم ساخته می‌شود، پس طبق ساختار با `total_price` reconcile می‌شود (`raw_gold + wage + tax + gift_box == grand_total`)
  - عمداً متد جداست و امضای `get_cart_items_with_pricing` دست‌نخورده مانده (چهار فراخوان دیگر دارد)
  - برای نمایندگان (که قیمت طلایی می‌بینند) نمایش داده نمی‌شود

### فرآیند ثبت سفارش (Checkout)
1. مشتری سبد خرید را بررسی می‌کند
2. روش تحویل انتخاب می‌شود:
   - **حضوری (Pickup)**: انتخاب نمایندگی از لیست
     - ⚠️ **تعداد موجودی هر نمایندگی به مشتری نمایش داده نمی‌شود** — فقط بج «موجود» یا «پیش‌سفارش». مقدار `stock` همچنان در پاسخ `/api/delivery/locations` ارسال و روی `data-stock` کارت ست می‌شود تا منطق سمت کلاینت کار کند، ولی رندر نمی‌شود
...
- **Snapshot محصول** — `product_id`, `product_name`, `product_weight` (گرم), `product_purity`, `applied_wage_percent`, `serial_code`
  - در لحظه فروش ثبت می‌شوند (هر دو مسیر: پوز نماینده و پوز مشتری)
  - دلیل: `bar_id` و `Bar.product_id` هر دو `ON DELETE SET NULL` هستند؛ بدون snapshot با حذف شمش/محصول اطلاعات محصول و وزن از گزارش پاک می‌شد
  - Properties کمکی: `display_product_name`, `display_serial`, `display_weight`, `weight_mg`, `wage_mg`, `our_profit_mg`

#### گزارش فروش نمایندگان (ادمین) — `/admin/dealers/sales`
- **کارت‌های خلاصه**: تعداد فروش، مجموع ریال، تعداد فروش با تخفیف، و به تفکیک طلا/نقره: مجموع وزن، سود فلز ما، سود فلز نمایندگان
- **جدول تفکیک محصول**: هر (محصول × فلز) → تعداد، مجموع وزن، مجموع ریال، سود فلز نماینده، سود فلز ما + ردیف جمع کل
- **جدول فروش‌ها**: در هر سطر نام محصول، وزن، عیار، درصد اجرت، سود فلز نماینده و سود فلز ما نمایش داده می‌شود
- سطرهایی که شمششان حذف شده با برچسب «شمش حذف شده» ولی با اطلاعات کامل محصول نمایش داده می‌شوند
- «سود فلز ما» = `wage_mg − metal_profit_mg` (هرگز منفی گزارش نمی‌شود)
...
- **تولید QR Code برای شمش‌ها**: QR Code با کیفیت بالا (۹۸۰×۱۰۶۰ پیکسل) با لوگوی برند و متن سریال‌کد — تولید لحظه‌ای (on-the-fly) بدون ذخیره روی دیسک، فقط از طریق endpoint احراز هویت‌شده ادمین (جزئیات کامل: [بخش ۳۲](#32-تولید-qr-code-برای-شمشها-qr-code-generation-for-bars))
...
| GET | `/admin/bars/{bar_id}/qr` | inventory:view | تولید لحظه‌ای و ارسال QR Code با کیفیت بالا (PNG) — بدون ذخیره روی دیسک |
...
### مشخصات دستگاه پوز (ادمین)

نمایندگانی که دستگاه کارتخوان دارند، مشخصات دستگاه‌شان ثبت می‌شود:
شماره پایانه، کد دستگاه، شماره سیم‌کارت، پین سیم‌کارت و فایل قرارداد.

- **هنگام ایجاد نماینده**: به‌صورت یک بخش اختیاری داخل همان فرم ایجاد (`POST /admin/dealers/create` — فرم `multipart/form-data`)
- **هنگام ویرایش**: کارت اختصاصی «مشخصات دستگاه پوز» با فرم مستقل (اندپوینت‌های زیر)
- مارک‌آپ فیلدها در `templates/admin/dealers/_pos_device_fields.html` مشترک است تا دو فرم از هم جدا نیفتند

| متد | مسیر | توضیح | دسترسی |
|------|------|--------|---------|
| POST | `/admin/dealers/{id}/pos-device` | ذخیره مشخصات پوز + آپلود قرارداد (PDF/تصویر، حداکثر ۱۰MB) | `dealers:edit` |
...
- کاربر با دسترسی فقط‌خواندنی (`dealers:view`) فیلدها را readonly می‌بیند و دکمه‌های ذخیره/حذف/آپلود برایش رندر نمی‌شود
- ذخیرهٔ فیلدهای متنی و ذخیرهٔ فایل **جدا** انجام می‌شوند: اگر فایل رد شود (فرمت/حجم)، مشخصات تایپ‌شده از دست نمی‌رود و فقط خطای فایل نمایش داده می‌شود. در مسیر ایجاد هم نماینده ساخته‌شده باقی می‌ماند و کاربر به صفحه ویرایش با پیام خطا هدایت می‌شود
...
سیستم برای هر شمش یک **QR Code با کیفیت بالا** به‌صورت لحظه‌ای (on-the-fly) تولید می‌کند که آماده چاپ لیزری روی بسته‌بندی وکیوم است. QR Code حاوی لینک صفحه احراز اصالت شمش (`/verify/check?code={serial_code}`) است و مشتری با اسکن آن می‌تواند اصالت شمش خود را بررسی کند.

> **امنیت**: QR Code هرگز روی دیسک ذخیره نمی‌شود. هر درخواست دانلود، QR را در لحظه تولید و به‌صورت stream ارسال می‌کند. فقط از طریق endpoint احراز هویت‌شده ادمین قابل دسترسی است.
...
| ذخیره‌سازی | بدون ذخیره — تولید لحظه‌ای در هر درخواست | هیچ فایلی روی دیسک ذخیره نمی‌شود |
| محتوای QR | `{BASE_URL}/verify/check?code={serial_code}` | لینک مستقیم به صفحه احراز اصالت |

### نحوه تولید

- **تولید لحظه‌ای (On-the-fly)**: هر بار که ادمین درخواست دانلود QR می‌دهد، تصویر در لحظه تولید و stream می‌شود. هیچ فایلی روی دیسک باقی نمی‌ماند.
- **بدون ذخیره روی دیسک**: پوشه `qrcodes/` وجود ندارد. این رویکرد از نشت QR Codeها از طریق دسترسی مستقیم به فایل‌های استاتیک جلوگیری می‌کند.
- **فقط پشت احراز هویت**: endpoint تولید QR فقط برای ادمین‌های دارای دسترسی `inventory:view` قابل استفاده است.
...
| فایل | توضیح |
|------|--------|
| `modules/verification/service.py` | `generate_qr_for_print()` — تولید QR با لوگو و متن سریال (بازگرداندن bytes بدون ذخیره روی دیسک) |
| `modules/inventory/admin_routes.py` | مسیر دانلود QR (تولید لحظه‌ای + StreamingResponse) |
| `templates/admin/inventory/edit_bar.html` | نمایش پیش‌نمایش QR + دکمه دانلود |

### دسترسی

| نقش | دسترسی |
|------|--------|
| Super Admin | مشاهده و دانلود QR Code از صفحه ویرایش شمش |
| Operator | بسته به سطح دسترسی `inventory` (view برای دانلود) |
...
### پیامک فروش به شماره‌های ثابت

- محل: `common/sales_alert.py` → `SALES_ALERT_MOBILES`، `notify_order_async()`، `notify_sale_async()`
- شماره‌ها هارد‌کد: `09121023589`، `09120725564`

| کانال فروش | محل تریگر | برچسب پیامک |
|-------------|-----------|--------------|
| فروشگاه اینترنتی | `order/service.py` → `finalize_order()` | «سفارش جدید #id» |
| پوز نماینده | `dealer/service.py` → `register_sale()` | «فروش پوز نماینده #id» |
| پوز مشتری (API) | `pos/service.py` → `confirm_sale()` | «فروش پوز مشتری #id» |
| دستگاه راسیس | `rasis/service.py` → sync رسیدها | «فروش دستگاه راسیس #id» |

- سفارش آنلاین فقط بعد از **پرداخت** پیامک می‌دهد (سفارش Pending/رهاشده نه)
- **غیر بلاکینگ**: متن همگام ساخته می‌شود (بعد از commit، آبجکت‌های ORM expire می‌شوند) و ارسال در یک **ترد دیمن** انجام می‌گیرد
- **فقط بعد از commit**: با هوک `after_commit` سشن. هوک `after_soft_rollback` هم ست می‌شود تا لیسنرِ تراکنشِ rollback‌شده روی سشن نماند و با کامیت بعدی شلیک نکند
- محتوای پیامک سفارش: شماره سفارش، نام/موبایل خریدار، اقلام + سریال، مبلغ (تومان یا گرم طلا)، روش تحویل
- محتوای پیامک POS: شماره فروش، نام نماینده، محصول + نوع فلز + سریال، مبلغ، نام/موبایل خریدار
- مستقل از سیستم `admin_alert_*` است (آن یکی از SystemSetting خوانده می‌شود و متن کوتاه هشدار موجودی می‌فرستد)
