...
- قیمت طلای ۱۸ عیار (از جدول Asset — بروزرسانی خودکار از goldis.ir)
- وزن و عیار محصول
...
تمام کاربران (مشتری، نماینده، ادمین) می‌توانند از طریق کیف پول اقدام به **خرید و فروش طلا** کنند. موجودی طلا بر حسب میلی‌گرم (XAU_MG) در حساب کاربر ذخیره می‌شود.

**کارمزد پویا بر اساس نقش:**
| نقش | کارمزد پیش‌فرض | کلید تنظیمات |
|------|----------------|---------------|
| مشتری | ~۲٪ | `gold_fee_customer_percent` |
| نماینده | ~۰.۵٪ | `gold_fee_dealer_percent` |

- کارمزد از صفحه تنظیمات ادمین قابل تغییر است (SystemSetting)
- در صفحه خرید/فروش طلا، کارمزد به صورت شفاف به کاربر نمایش داده می‌شود
...
- **خرید طلا**: تبدیل موجودی ریالی به طلا (میلی‌گرم) با کارمزد پویا بر اساس نقش کاربر
- **فروش طلا**: تبدیل موجودی طلا (میلی‌گرم) به ریال با کارمزد پویا بر اساس نقش کاربر
...
- **مشاهده تاریخچه**: لیست تمام تراکنش‌ها با فیلتر نوع دارایی (`asset`: all/irr/gold)
- **شفافیت کارمزد**: نمایش درصد و مبلغ کارمزد قبل از تایید معامله طلا

### آدرس‌ها (همه کاربران — `require_login`)
| متد | مسیر | توضیح |
|------|------|--------|
| GET | `/wallet` | داشبورد کیف پول |
| GET | `/wallet/transactions` | تاریخچه تراکنش‌ها (فیلتر: asset=all/irr/gold) |
...
| GET | `/wallet/gold` | صفحه خرید/فروش طلا |
| POST | `/wallet/gold/buy` | خرید طلا (ریال → میلی‌گرم طلا) |
| POST | `/wallet/gold/sell` | فروش طلا (میلی‌گرم طلا → ریال) |
...
| base_gold_price_18k | قیمت طلای ۱۸ عیار (ریال/گرم) — از جدول Asset | 52,000,000 |
...
| ویرایش و ارسال مجدد درخواست نمایندگی (RevisionNeeded) | ✅ | — | — | — |
| بررسی درخواست‌های نمایندگی (تایید/رد/بازبینی) | — | — | ✅ | ✅ |
...
| ماژول | تابع | رفتار |
|--------|--------|--------|
| `order/service.py` | `checkout()` | خطا: «قیمت به‌روز نیست» |
| `dealer/service.py` | `create_pos_sale()` | خطا: «قیمت به‌روز نیست» |
| `pos/service.py` | `reserve_bar()` / `confirm_sale()` | خطا: «قیمت به‌روز نیست» |
...
| `modules/pricing/service.py` | توابع `get_price_value`, `is_price_fresh`, `require_fresh_price`, `update_asset_price` |
