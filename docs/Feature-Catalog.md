...
### پیامک سفارش به شماره‌های ثابت

- محل: `modules/order/service.py` → `ORDER_ALERT_MOBILES`، `build_order_alert_text()`، `notify_order_alert_async()`
- تریگر: داخل `finalize_order()` — یعنی فقط سفارش‌های **پرداخت‌شده** (سفارش Pending/رهاشده پیامک نمی‌دهد)
- شماره‌ها هارد‌کد: `09121023589`، `09120725564`
- غیر بلاکینگ: متن همگام ساخته می‌شود (تا از expire شدن آبجکت‌ها بعد از commit جلوگیری شود) و ارسال در یک **ترد دیمن** انجام می‌گیرد
- محتوای پیامک: شماره سفارش، نام/موبایل خریدار، اقلام + سریال شمش، مبلغ (تومان یا گرم طلا برای سفارش طلایی)، روش تحویل (حضوری + نماینده / پستی + استان‌وشهر)
