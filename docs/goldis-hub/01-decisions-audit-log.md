...
<tr><td>D-06d</td><td>**Gold settle بدون bar مشخص**</td><td>وقتی Goldis طلای خام را به TalaMala تحویل می‌دهد، فقط مقدار وزن (mg) ثبت می‌شود — نه bar خاص. اپراتور می‌گوید «۱۰g طلای خام تحویل دادم» و تعهد طلا تسویه می‌شود (مکانیکِ تسویه با [D-102](01-decisions-audit-log.md) از FIFO consume به **append یک ردیف NET جهت‌مخالف** تغییر کرد — دیگر FIFO نیست). این طلای خام (گرانول/شمش بزرگ) برای hedging موجودی فروشنده یا تولید بعدی است، **نه refill همان شمش فروختهشده**.</td><td>تیم</td></tr>
...
## ۲.۷. تصمیمات بازبینی پیش از پیاده‌سازی (Pre-Build Review — D-100…D-110)

> این تصمیمات از یک بازبینی معماری عمیق **قبل از شروع کدنویسی** استخراج شده‌اند (انسانی + multi-agent با تحقیق best-practice و محصولات مشابه واقعی). چند مورد، تصمیمات قبلی را **override یا تکمیل** می‌کنند (مرجع در ستون «اثر/override»).
>
...
<table>
<thead>
<tr><th>#</th><th>تصمیم</th><th>اثر / override / فایل‌های هدف</th></tr>
</thead>
<tbody>

<tr><td>D-100</td><td>**خزانه = signed-sum خالص (حذف مدل coverage)** — مدل exposure فقط «جمع علامت‌دار» است: `exposure(metal) = SUM(treasury_positions.delta_amount_mg WHERE metal_type=? AND status='open')`. `hedge_buy` یک ردیف با delta **منفی** است (D-90). مفهوم coverage کاملاً **حذف**: ستون‌های `covered_amount_mg` و `covered_at`، مقادیر status `partially_covered`/`covered`، و endpoint `POST /admin/treasury/positions/{id}/cover` حذف می‌شوند. `status` فقط دو مقدار: `open` (در exposure شمرده می‌شود) و `cancelled` (نادیده).</td><td>**Override** بخش coverage در D-14/D-47/D-67. منطبق با ماژول کارکرده‌ی `hedging/` در v4. هدف: `03 §3` (treasury_positions)، `02 §5.2`، `04` (حذف cover endpoint)، `references/finance §2.2`.</td></tr>

<tr><td>D-101</td><td>**سقف خزانه دوسطحی + فرمول canonical + قفل صحیح** — چک سقف سخت inline (D-47) دوسطحی است: در checkout یک `inventory_pending_holds` نوشته می‌شود؛ در PaymentVerified به یک `treasury_position` تبدیل (finalize) می‌شود. **فرمول canonical (تنها مرجع):** `committed + reserved + this_tx ≤ cap` که `committed = SUM(open positions)` و `reserved = SUM(pending_holds زنده)`. **همزمانی:** ناحیه‌ی بحرانی با `pg_advisory_xact_lock(hashtext('treasury:'||metal_type))` سریالایز می‌شود؛ `treasury_settings` با `SELECT` ساده داخل همین ناحیه خوانده می‌شود (نه `SELECT FOR UPDATE` که کل فروش را روی یک ردیف می‌بندد و ویرایش زنده‌ی سقف را بلاک می‌کند). ناحیه‌ی قفل کوچک = (read committed + read reserved + compare + INSERT hold) و **هرگز** روی round-trip درگاه نگه داشته نمی‌شود. **ترتیب قفل سراسری یکسان** در flows 01/04/07/08: `advisory(treasury per metal) → wallet rows(asset_code,id) → bar(id)`. یک استراتژی قفل per hot row (pessimistic/advisory)؛ `asset_balances.version` یا حذف یا به‌عنوان optimistic guard غیرمالی مستند شود.</td><td>**Override** خط ۳۲۲۳ سند منبع و P0-5 (lock target). **تکمیل** D-47/D-97. هدف: `03 §3+§14`، `02 §5.3`، `references/finance §2.3`، `flows 01/04/07/08`.</td></tr>

<tr><td>D-102</td><td>**دفترکل بین‌شرکتی = حساب جاری علامت‌دار net (حذف FIFO/status)** — `inter_company_ledger` یک دفتر علامت‌دارِ append-only است. **تعهد معوق = `NET SUM(signed amount_minor) per (creditor, debtor, asset_type)`**. حذف: `status` (open/partial/settled)، `settled_amount_minor`، `settled_at`، `settled_by`، `chk_settled_le_amount`، index جزئی `WHERE status IN ('open','partial')`، و `inter_company_settle_actions.affected_ledger_ids[]` (کل مفهوم FIFO consume). **تسویه = append یک ردیف settlement** که net را به سمت صفر می‌برد (نه mutate ردیف‌های قبلی). جفت‌های جهت‌مخالف (digital buy↔sell، offset کمیسیون D-84، buyback) **خودکار در net صفر می‌شوند** — هیچ تعهد مستقل دوم ساخته نمی‌شود. گزارش `balance` و عملیات settle هر دو از همین net می‌خوانند.</td><td>**Override** مکانیک FIFO/status در D-06b/D-59/D-70/D-84/D-85 (مدل اقتصادی hub-and-spoke دست‌نخورده). هدف: `03 §4`، `02 §6.4/6.5`، `flows 12+15`، `references/finance §3.5`، `04` (settle endpoints)، `references/testing §3.4`.</td></tr>

<tr><td>D-103</td><td>**نهایی‌سازی پرداخت اتمیک؛ `payment_state` فقط observability** — finalize یک **transaction اتمیک** است: `gateway-verify` خارج از transaction؛ سپس داخل یک transaction: `wallet ledger → treasury position (from hold) → inter-company net rows → outbox events → order.status=Paid`. بازیابی crash = **retry idempotent کل عملیات** با کلید کسب‌وکاریِ پایدار (نه saga). `payment_states` و D-98 صرفاً observability/alerting‌اند؛ **هیچ شاخه‌ی control-flow نباید `payment_state` را برای resume یک saga بخواند.** هر دو رویداد `InterCompanyObligationCreated` در همان transaction درج می‌شوند.</td><td>**Override** D-92/D-98 (به observability تقلیل). حل تناقض با نمونه‌ی outbox در `05`. منطبق با `finalize_order` اثبات‌شده‌ی v4. هدف: `03 §12`، `05`، `flows 01/04/07/09`.</td></tr>

<tr><td>D-104</td><td>**میلی‌گرم صحیح در همه‌جا + سیاست گرد‌کردن واحد FLOOR** — همه‌ی مقادیر طلا/نقره `BIGINT` میلی‌گرم صحیح. دو فیلد `NUMERIC(12,4)` (`payment_reconciliations.treasury_adjustment_mg` و `inventory_pending_holds.pure_gold_mg_reserved`) به `BIGINT` تبدیل می‌شوند. تبدیل `pure_gold_mg = floor(weight_mg × purity / 1000)` با سیاست **FLOOR** (محافظه‌کارانه؛ هیچ‌وقت بیش از پشتوانه طلا اعتبار/تعهد ساخته نمی‌شود) در **یک helper واحد**؛ مقدار pre-round + شناسه‌ی سیاست در `metadata` ذخیره می‌شود. **Invariant بحرانی:** هر سه پای یک تراکنش (treasury delta، wallet credit، inter-company obligation) از **همان** عدد صحیح گردشده مشتق می‌شوند تا تراکنش دوپایی **دقیقاً به ۰ mg** برسد.</td><td>**تکمیل** D-30/D-51. تست واحد: `sum-of-legs == 0 mg` برای هر flow دوپایی. هدف: `03` (خطوط 1142/1157 + هرجای دیگر)، pricing calculator.</td></tr>

<tr><td>D-105</td><td>**انقضای pending holds (رفع نشت سقف)** — `inventory_pending_holds` ستون `expires_at TIMESTAMPTZ NOT NULL` می‌گیرد (مثل سایر جداول reservation). فرمول سقف فقط hold‌های زنده را جمع می‌زند: `WHERE released_at IS NULL AND finalized_at IS NULL AND expires_at > now()`. worker `lock_expirer` hold‌های منقضی را با **UPDATE شرطیِ state/version-guarded** آزاد می‌کند (نه blind delete — تا یک hold نتواند همزمان finalize و release شود).</td><td>**رفع باگ** کشف‌شده در بازبینی: checkout رهاشده تا ابد سقف را مصرف می‌کرد و فلز را قفل می‌نمود. هدف: `03 §14`، `references/outbox-workers (lock_expirer)`، فرمول D-101.</td></tr>

<tr><td>D-106</td><td>**worker آشتی‌سازی + invariantهای solvency (جزء هسته‌ی مالی)** — یک worker زمان‌بندی‌شده (APScheduler)، **بخشی از هسته‌ی مالی (نه فاز ۶)**، دوره‌ای: (۱) سه self-reconciliation: `balance == Σ wallet_ledger`، `open exposure == Σ signed delta_amount_mg`، `inter-company outstanding == NET per (creditor,debtor,asset)`؛ (۲) اتحاد cross-ledger solvency: `open_exposure == net_digital_liabilities + net_intercompany_gold_owed − hedged_raw_gold`؛ (۳) snapshot در `treasury_position_snapshots` + **incident/alert روی هر residue غیرصفر**؛ (۴) reconciliation سه‌طرفه‌ی external (bank/gateway برای IRR، لیست فیزیکی شمش برای gold، net طرف‌مقابل برای inter-company) + workflow بررسی break؛ (۵) cycle-count کور شمش‌های امانی (`delivered_at IS NULL`) در برابر خزانه.</td><td>**رفع شکاف** بحرانی: بدون این، drift (rounding/ویرایش دستی/نوشتن ناقص) نامرئی می‌ماند تا موج redemption (failure-mode سینَپس) — و شرط «پشتوانه‌ی واقعی طلا»ی بانک مرکزی نقض می‌شود. هدف: `references/outbox-workers`، `references/testing`، انتقال در roadmap به فاز ۱-۲.</td></tr>

<tr><td>D-107</td><td>**اجبار تفکیک وظایف (maker ≠ checker)** — SoD از «توصیه/audit» به **اجبار** ارتقا می‌یابد: `inventory_transfer_documents` → `CHECK(dispatched_by IS NULL OR received_by IS NULL OR dispatched_by <> received_by)`؛ trio بازخرید (`received_by`/`verified_by`/`approved_by`) → گارد لایه‌ی app که نقش‌های متوالی نباید یک `user_id` باشند، **قبل** از هر منطق approval؛ گارد سراسری `if maker_id == checker_id: raise` روی همه‌ی workflowهای dual-control (transfer/buyback/withdrawal)؛ **`super_admin` از actor بودن در گام‌های تأیید مالی منع می‌شود** (bypass permission ≠ bypass SoD)؛ گزارش کارآگاهی دوره‌ای روی `audit_logs` برای same-actor-both-halves و same-device.</td><td>**رفع شکاف**: invariant اعلام‌شده ولی هیچ‌جا enforce نشده بود. هدف: `03 §14` (CHECK)، `05` (RBAC/SoD)، `flows 06/13/10`، `references/identity §3`.</td></tr>
...
3. **روند settlement (v1 — Manual)**:
   - **ماهانه**, اپراتور:
     ...
     - سیستم: یک ردیف settlement علامت‌دارِ جهت‌مخالف روی همان (creditor, debtor, asset='gold') append می‌کند؛ تعهد hedge و تعهد commission به‌صورت **NET** خودکار به سمت صفر می‌روند (بدون FIFO consume، بدون mutate ردیف‌های قبلی — D-102)
