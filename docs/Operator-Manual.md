...
10.5. [سناریو ۹.۵: پیگیری تحویل فیزیکی و طلای امانی](#105-سناریو-۹۵-پیگیری-تحویل-فیزیکی-و-طلای-امانی)
...
4. فرم را پر کنید:
   ...
   - **نوع فلز**: طلا یا نقره — تعیین می‌کند قیمت‌گذاری محصول بر اساس کدام فلز انجام شود
   - **وزن**: وزن شمش (گرم)
   - **عیار**: خلوص فلز (طلا: ۷۵۰ = ۱۸ عیار، نقره: ۹۹۹)
   - **اجرت ساخت**: درصدی (از ارزش فلز خام)
5. **«ذخیره»** را بزنید
...
**توجه**: شمش‌ها معمولاً از طریق ایمپورت دسته‌ای وارد سیستم می‌شوند. مدیریت تکی شمش‌ها از طریق صفحه ویرایش انجام می‌شود.

#### ایمپورت سریال از فایل اکسل (`scripts/import_serials.py`)

**کی لازم است؟** وقتی لیستی از سریال شمش‌های چاپ‌شده دارید (مثلاً شمش‌هایی که قبلاً فروخته شده‌اند) و می‌خواهید در سیستم ثبت شوند تا در صفحه استعلام اصالت پاسخ بدهند.

**ساختار فایل اکسل**: نام هر شیت = `product_id`، و ستونی با هدر `serial` که سریال‌ها در آن است. ستون‌های دیگر نادیده گرفته می‌شوند.

```bash
cd /path/to/talamala_v4
source venv/bin/activate

python scripts/import_serials.py                 # فقط گزارش می‌دهد، چیزی نمی‌نویسد
python scripts/import_serials.py --commit        # اجرای واقعی
```

**همیشه اول بدون `--commit` اجرا کنید.** خروجی دقیقاً نشان می‌دهد چه چیزی ساخته می‌شود، چه سریالی از قبل هست و چه خطایی وجود دارد — بدون هیچ نوشتنی در دیتابیس.

گزینه‌ها:
- `-f FILE` — فایل ورودی (پیش‌فرض `serials.xlsx`)
- `-c COLUMN` — نام ستون سریال (پیش‌فرض `serial`)
- `--status` — وضعیت شمش‌های ساخته‌شده (پیش‌فرض `Sold`)

**شمش‌های ایمپورت‌شده چه شکلی‌اند؟** وضعیت «فروخته»، بدون مالک، بدون نمایندگی، و `is_sellable = false`. یعنی در موجودی فروشگاه و هیچ POSای دیده نمی‌شوند — فقط در استعلام اصالت پاسخ می‌دهند.

**رفتار ایمن:**
- اگر `product_id` (نام شیت) در دیتابیس نباشد، **هیچ‌چیز نوشته نمی‌شود** و خطا گزارش می‌شود
- سریال تکراری داخل فایل یا سریالی که از قبل در دیتابیس هست → گزارش و رد شدن، بدون خطا
- سریال بلندتر از ۸ کاراکتر یا با کاراکتر نامعتبر → خطا و توقف کامل
- همه‌چیز در **یک تراکنش** نوشته می‌شود: اگر یک ردیف مشکل داشته باشد، هیچ ردیفی ذخیره نمی‌شود
- اجرای دوباره بی‌خطر است — سریال‌های موجود دوباره ساخته نمی‌شوند
...
2. فیلترهای موجود:
   ...
   - **قابل فروش**: همه / فقط قابل فروش / فقط غیرقابل فروش
   - **جستجو**: بر اساس سریال‌کد
3. هر ردیف نمایش می‌دهد: سریال‌کد، وضعیت، **قابل فروش**، محصول، بچ، نمایندگی، مشتری
...
2. می‌توانید تغییر دهید:
   ...
   - **قابل فروش**: آیا این شمش در معرض فروش قرار بگیرد یا نه
3. **ذخیره** کنید

### ۲.۴.۰ عملیات گروهی — تغییر وضعیت چند شمش

**کی لازم است؟** وقتی یک بچ شمش تازه از کارخانه رسیده و می‌خواهید همه را یکجا از «خام» به «اختصاص» ببرید.

1. شمش‌های موردنظر را تیک بزنید → **«عملیات گروهی»**
2. در بالای مودال، **وضعیت** را انتخاب کنید:
   - **بدون تغییر** (پیش‌فرض): وضعیت خودکار از روی محصول/مالک تعیین می‌شود. اگر محصول و مالک را هم تغییر ندهید، وضعیت **دست‌نخورده** می‌ماند
   - **خام / اختصاص / فروخته**: وضعیت را صریحاً تعیین می‌کند و بر تعیین خودکار اولویت دارد
3. در صورت نیاز محصول، مالک، بچ و نمایندگی را هم تنظیم کنید
4. **«اعمال تغییرات»**

> **نکته مهم**: اگر فقط نمایندگی را عوض کنید و وضعیت را «بدون تغییر» بگذارید، شمش خام همچنان خام می‌ماند و هرگز فروخته نمی‌شود. برای وارد کردن شمش به چرخه فروش، وضعیت را صریحاً روی **«اختصاص»** بگذارید (و بعد قابلیت فروش را فعال کنید — بخش بعدی).

**سیستم جلوی وضعیت‌های ناسازگار را می‌گیرد** و پیام خطا با تعداد شمش‌های مشکل‌دار نشان می‌دهد:

| وضعیت هدف | چه چیزی لازم است |
|---|---|
| **خام** | شمش نباید محصول یا مالک داشته باشد — باید «حذف محصول» و «حذف مالک» را هم انتخاب کنید |
| **اختصاص** | همه شمش‌ها باید نمایندگی و محصول داشته باشند |
| **فروخته** | علاوه بر نمایندگی و محصول، باید مالک هم داشته باشند |

اگر خطا بگیرید، **هیچ تغییری اعمال نشده** — انتخاب‌ها را اصلاح کنید و دوباره بزنید.

**چرا «رزرو» در لیست نیست؟** رزرو را سبد خرید یا POS می‌سازد و همراهش مشخص می‌کند چه کسی رزرو کرده و تا کی. اگر گروهی رزرو کنید، شمشی می‌سازید که رزرو است ولی نه دارنده دارد نه تاریخ انقضا — سیستم پاک‌سازی خودکار هرگز آزادش نمی‌کند و برای همیشه از موجودی خارج می‌شود. اگر واقعاً لازم دارید، از فرم ویرایش تکی استفاده کنید.

### ۲.۴.۱ مدیریت قابلیت فروش (is_sellable) ⭐

> **مهم‌ترین نکته**: عددی که به‌عنوان **موجودی** در سایت، سبد خرید، چک‌اوت و همه دستگاه‌های فروش (POS نماینده، POS مشتری، Rasis) نمایش داده می‌شود، دقیقاً برابر **تعداد شمش‌های قابل فروش** است — نه تعداد کل شمش‌های Assigned.

**شمش جدید به‌صورت پیش‌فرض غیرقابل فروش است.** تا وقتی صراحتاً فعالش نکنید، در هیچ کانالی به فروش نمی‌رود.

**چرا این قابلیت لازم است؟** شمشی ممکن است فیزیکاً در انبار باشد ولی نخواهید بفروشیدش:
- شمش رزرو فیزیکی برای مشتری خاص
- شمش دارای ایراد کیفی یا در انتظار بررسی
- شمش نمایشگاهی / ویترین
- نگه‌داشتن موجودی برای مدیریت ریسک قیمت

**فعال یا غیرفعال کردن گروهی:**

1. در لیست شمش‌ها، تیک شمش‌های موردنظر را بزنید (یا تیک بالای ستون را برای انتخاب کل صفحه)
2. روی **«عملیات گروهی»** کلیک کنید — تعداد انتخاب‌شده کنار دکمه نمایش داده می‌شود
3. در بخش **«قابلیت فروش»**:
   - **«قابل فروش کن»** → شمش‌ها وارد چرخه فروش می‌شوند
   - **«غیرقابل فروش کن»** → شمش‌ها از فروش خارج می‌شوند (یک پیام تأیید نمایش داده می‌شود)
4. پیام نتیجه تعداد شمش‌های تغییریافته را نشان می‌دهد

**نکات عملیاتی:**
- اگر شمشی از قبل در وضعیت موردنظر باشد، دوباره شمرده نمی‌شود (پیام «۰ شمش ...»)
- غیرفعال‌کردن شمشی که در سبد خرید مشتری **رزرو** است، رزرو را باطل نمی‌کند — آن سفارش تکمیل خواهد شد
- اگر نمایندگی دستگاه Rasis داشته باشد، فعال/غیرفعال‌کردن به‌صورت خودکار روی دستگاه هم اعمال می‌شود
- برای دیدن سریع اینکه چه چیزی در معرض فروش است، از فیلتر **«فقط قابل فروش»** استفاده کنید
- این عملیات نیازمند سطح دسترسی `inventory:full` است
...
```
  خام (Raw)
    │
    ▼  ← تخصیص محصول
  تخصیص (Assigned)  ← در انبار، ولی هنوز در معرض فروش نیست
    │
    ▼  ← فعال‌سازی «قابل فروش» (is_sellable = true)
  قابل فروش  ← در موجودی سایت و همه POSها شمرده می‌شود
    │
...
### ۴.۳ توزیع شمش از مرکز پخش

> نمایندگانی که **انبار مرکزی** هستند (`is_warehouse=True`) می‌توانند از پنل خود شمش‌ها را مستقیماً به نمایندگان دیگر منتقل کنند.

1. با حساب نماینده مرکز پخش وارد شوید
2. از منوی سایدبار **«انتقال به نماینده»** را انتخاب کنید
3. **نماینده مقصد** را از لیست انتخاب کنید
4. شمش‌های مورد نظر را با **چک‌باکس** انتخاب کنید (یا «انتخاب همه»)
5. در صورت نیاز **توضیحات** اضافه کنید (مثلاً: ارسال با پست)
6. **«انتقال شمش‌های انتخاب‌شده»** را بزنید و تأیید کنید
7. شمش‌ها فوراً به موجودی نماینده مقصد منتقل می‌شوند

**تاریخچه**: تب «تاریخچه» تمام انتقالات ارسالی و دریافتی را نشان می‌دهد.
...
5. سیستم به‌صورت خودکار اقدامات زیر را انجام می‌دهد:
   ...
   - مالکیت شمش حذف می‌شود (user_id → null)
...
- اگر شمش فاقد user_id (مالک) باشد: بازگشت اجرت انجام نمی‌شود.
...
2. تنظیمات قابل تغییر:
   ...
   - **کارمزد معاملات طلا**:
     - **کارمزد مشتری عادی** (`gold_fee_customer_percent`): کارمزد خرید/فروش طلا برای مشتریان — پیش‌فرض ۲٪
     - **کارمزد نماینده** (`gold_fee_dealer_percent`): کارمزد خرید/فروش طلا برای نمایندگان — پیش‌فرض ۰.۵٪
   - **کارمزد معاملات نقره**:
     - **کارمزد مشتری نقره** (`silver_fee_customer_percent`): کارمزد خرید/فروش نقره برای مشتریان — پیش‌فرض ۱.۵٪
     - **کارمزد نماینده نقره** (`silver_fee_dealer_percent`): کارمزد خرید/فروش نقره برای نمایندگان — پیش‌فرض ۰.۳٪
3. **«ذخیره»** را بزنید

**هشدار**: تغییر قیمت هر فلز فوراً روی قیمت محصولات آن فلز در فروشگاه اثر می‌گذارد.

**نکته مهم — انقضای قیمت هر فلز به‌صورت مستقل**: بررسی تازگی قیمت **به‌ازای هر فلز** انجام می‌شود. یعنی:
- اگر قیمت **نقره** منقضی شده ولی طلا تازه باشد → فقط عملیات مربوط به محصولات نقره‌ای بلاک می‌شود
- اگر قیمت **طلا** منقضی شده ولی نقره تازه باشد → فقط عملیات مربوط به محصولات طلایی بلاک می‌شود
- در **سفارشات مختلط** (طلا + نقره در یک سبد) → تازگی **هر دو** فلز بررسی می‌شود و اگر حتی یکی منقضی باشد، سفارش ثبت نمی‌شود

**ادمین باید قیمت هر دو فلز (طلا و نقره) را تازه نگه دارد** مخصوصاً در زمان‌هایی که سفارشات مختلط رایج است.

عملیات بلاک‌شونده در صورت انقضای قیمت فلز مربوطه:
- خرید آنلاین (checkout) بلاک می‌شود
- فروش حضوری (POS) بلاک می‌شود
- تبدیل کیف پول (ریال ↔ طلا/نقره) بلاک می‌شود
...
2. پس از پرداخت موفق:
   ...
   - `user_id` شمش **NULL** می‌ماند (به نام خریدار ثبت نمی‌شود)
...
| شکایت «شمش انتقال نمی‌شود» | بررسی کنید آیا شمش واقعاً به نام مشتری ثبت شده (وضعیت SOLD + user_id) |
| شکایت «کد ثبت مالکیت گم شده» | در لیست شمش‌ها سریال را جستجو کنید — claim_code قابل مشاهده است |

---

## 10.5. سناریو ۹.۵: پیگیری تحویل فیزیکی و طلای امانی

### ۱۰.۵.۱ آشنایی

سیستم وضعیت تحویل فیزیکی هر شمش را به‌صورت مستقل (بر اساس شمش، نه سفارش) پیگیری می‌کند. شمشی که فروخته شده اما هنوز تحویل فیزیکی نشده، **طلای امانی** (Custodial Gold) محسوب می‌شود.

**فیلد کلیدی**: `delivered_at` روی هر شمش — اگر خالی (NULL) باشد یعنی شمش هنوز تحویل فیزیکی نشده.

### ۱۰.۵.۲ طلای امانی چیست؟

**طلای امانی** = شمشی که:
- وضعیت **فروخته شده** (SOLD) دارد
- هنوز **تحویل فیزیکی نشده** (`delivered_at` خالی است)

این حالت در موارد زیر رخ می‌دهد:
- سفارش آنلاین پرداخت شده اما هنوز ارسال/تحویل نشده
- سفارش هدیه پرداخت شده اما گیرنده هنوز شمش را دریافت نکرده
- شمشی که مالکیتش منتقل شده اما فیزیکی هنوز نزد مالک جدید نیست

### ۱۰.۵.۳ چه زمانی تحویل ثبت می‌شود؟

| روش فروش | زمان ثبت `delivered_at` |
|-----------|------------------------|
| **فروش حضوری (POS)** | بلافاصله هنگام ثبت فروش — چون شمش در محل تحویل می‌شود |
| **سفارش آنلاین — حضوری** | وقتی ادمین وضعیت تحویل سفارش را به «تحویل‌شده» تغییر می‌دهد |
| **سفارش آنلاین — پستی** | وقتی ادمین وضعیت تحویل سفارش را به «تحویل‌شده» تغییر می‌دهد |

**نکته مهم**: انتقال مالکیت (Transfer) تاثیری بر وضعیت تحویل ندارد. اگر مشتری A شمشی را به مشتری B منتقل کند، شمش همچنان **امانی** باقی می‌ماند تا تحویل فیزیکی انجام شود.

### ۱۰.۵.۴ مشاهده وضعیت تحویل شمش

**در لیست شمش‌ها** (`/admin/bars`):
- شمش‌های فروخته‌شده بدون تحویل فیزیکی (`delivered_at` خالی) = طلای امانی
- شمش‌های تحویل‌شده (`delivered_at` مقداردهی‌شده) = تحویل فیزیکی انجام شده

### ۱۰.۵.۵ نکات مهم برای اپراتور

| موقعیت | نکته |
|---------|-------|
| تایید تحویل سفارش | با تغییر وضعیت تحویل سفارش به «تحویل‌شده»، تمام شمش‌های آن سفارش به‌صورت خودکار تحویل‌شده علامت‌گذاری می‌شوند |
| فروش حضوری (POS) | نیازی به اقدام اضافی نیست — تحویل خودکار ثبت می‌شود |
| انتقال مالکیت | شمش پس از انتقال همچنان امانی است — تا تحویل فیزیکی انجام نشده، `delivered_at` خالی باقی می‌ماند |
| شکایت مشتری درباره طلای امانی | بررسی کنید آیا سفارش تحویل‌شده ثبت شده یا خیر — اگر ارسال شده اما تحویل ثبت نشده، وضعیت تحویل سفارش را به‌روز کنید |
...
| **سود فلزی کل** | مجموع سود فلزی نمایندگان (بر حسب میلی‌گرم) — به‌تفکیک نوع فلز (طلا / نقره) بر اساس فیلد `metal_type` هر فروش |
...
| **نوع فلز** | نوع فلز محصول فروخته‌شده: طلا یا نقره (`metal_type`) — برای تفکیک آمار و گزارش‌گیری |
| **سود فلزی** | سود فلزی نماینده از این فروش (میلی‌گرم) — بر اساس نوع فلز به کیف پول مربوطه واریز می‌شود (XAU_MG برای طلا، XAG_MG برای نقره) |
...
## ۱۵.۶ بکاپ شبانه دیتابیس (`scripts/backup_db.sh`)

**کی لازم است؟** یک بار راه‌اندازی می‌شود و بعد هر شب خودکار اجرا می‌شود.

اسکریپت اعتبارنامه‌ها را از همان `.env` پروژه می‌خواند، پس جایی تکرار نمی‌شوند.

### راه‌اندازی

```bash
# ۱. کلاینت پستگرس (اگر نصب نیست)
sudo apt install postgresql-client -y

# ۲. اجرای دستی برای اطمینان
cd /path/to/talamala_v4
./scripts/backup_db.sh

# ۳. افزودن به cron — هر شب ساعت ۳ بامداد
sudo crontab -e
```

خط زیر را اضافه کنید (مسیر را با مسیر واقعی پروژه عوض کنید):
...
```bash
./scripts/backup_db.sh              # گرفتن بکاپ
./scripts/backup_db.sh --list       # لیست بکاپ‌های موجود
tail -50 /var/log/talamala-backup.log   # دیدن نتیجه اجراهای شبانه
```
...
| متغیر | پیش‌فرض | توضیح |
|---|---|---|
| `BACKUP_DIR` | `/var/backups/talamala` | محل ذخیره |
| `RETENTION_DAYS` | `14` | بکاپ‌های قدیمی‌تر از این حذف می‌شوند |
| `ENV_FILE` | `.env` پروژه | مسیر فایل تنظیمات |

مثال: `RETENTION_DAYS=30 ./scripts/backup_db.sh`

### بازیابی

فرمت `custom` است، پس با `pg_restore` بازیابی می‌شود (نه `psql`):

```bash
# بازیابی کامل روی یک دیتابیس تازه (امن‌ترین راه)
createdb -U postgres talamala_restore
pg_restore -U postgres -d talamala_restore --no-owner /var/backups/talamala/talamala_v4_2026-07-18_0300.dump

# فقط یک جدول
pg_restore -U postgres -d talamala_v4 --no-owner --data-only -t bars <فایل>

# دیدن محتویات بدون بازیابی
pg_restore --list <فایل>
```

> **هرگز مستقیماً روی دیتابیس زنده restore نکنید.** اول روی یک دیتابیس جداگانه بازیابی کنید، بررسی کنید داده درست است، بعد تصمیم بگیرید.

### رفتار ایمن

- اگر `pg_dump` شکست بخورد، **هیچ فایلی ساخته نمی‌شود** — فایل ناقص با پسوند `.part` نوشته و در صورت خطا پاک می‌شود
- بعد از هر بکاپ، فایل با `pg_restore --list` بررسی می‌شود؛ اگر خوانا نباشد یا هیچ جدولی نداشته باشد دور ریخته می‌شود
- قفل فایلی مانع اجرای همزمان دو بکاپ می‌شود
- فایل‌ها با دسترسی `600` و پوشه با `700` ساخته می‌شوند

> ⚠️ **بکاپ روی همان سرور، بکاپ واقعی نیست.** اگر دیسک یا سرور از دست برود، بکاپ‌ها هم می‌روند. حتماً یک کپی خارج از سرور نگه دارید (بخش زیر).

### کپی خارج از سرور (توصیه جدی)

بعد از خط cron بالا، این را هم اضافه کنید تا بکاپ شبانه به سرور دیگری منتقل شود:

```cron
30 3 * * * rsync -az --delete /var/backups/talamala/ user@backup-server:/backups/talamala/ >> /var/log/talamala-backup.log 2>&1
```
...
**دلیل**: قیمت فلز مربوطه (طلا یا نقره) بیش از آستانه انقضا به‌روز نشده است. بررسی تازگی قیمت **per-metal** انجام می‌شود — اگر محصول نقره‌ای می‌فروشید باید قیمت نقره تازه باشد، و اگر طلایی می‌فروشید باید قیمت طلا تازه باشد.

**راه‌حل**:
1. به **تنظیمات** بروید و وضعیت نشان تازگی قیمت **هر دو فلز** (طلا و نقره) را بررسی کنید
2. اگر نشان **قرمز** است: دکمه **«بروزرسانی دستی»** را برای آن فلز بزنید
3. بررسی کنید سوئیچ **بروزرسانی خودکار** روشن باشد
4. **نکته**: برای سفارشات مختلط (طلا + نقره) هر دو قیمت باید تازه باشند
5. اگر مشکل ادامه دارد: ممکن است سایت goldis.ir در دسترس نباشد — قیمت را دستی وارد کنید
...
**دلایل احتمالی**:
1. شمش به نام کاربر ثبت نیست (user_id خالی)
...
| **طلای امانی (Custodial Gold)** | شمشی که فروخته شده اما هنوز تحویل فیزیکی نشده — `delivered_at` خالی (NULL) است |
| **تحویل فیزیکی (delivered_at)** | تاریخ و ساعت تحویل واقعی شمش به مشتری — در فروش POS بلافاصله، در سفارش آنلاین با تایید تحویل ادمین ثبت می‌شود |
...
| **سود فلزی (Metal Profit)** | سود نماینده از فروش حضوری بر حسب میلی‌گرم فلز مربوطه (طلا یا نقره) — حاصل اختلاف اجرت نماینده و اجرت مشتری نهایی. به کیف پول فلز مربوطه واریز می‌شود (XAU_MG برای طلا، XAG_MG برای نقره) |
...
### تایید درخواست نمایندگی — چه اتفاقی می‌افتد؟

با زدن دکمه «تایید» در `/admin/dealer-requests/{id}`:

1. وضعیت درخواست به «تایید شده» تغییر می‌کند و پیامک/اعلان برای متقاضی می‌رود
2. **حساب کاربری متقاضی به نماینده تبدیل می‌شود** و از همان لحظه به پنل `/dealer/dashboard` دسترسی دارد
3. نام، استان و شهر و همچنین **عکس جواز کسب و عکس مغازه** از درخواست به پروندهٔ نماینده منتقل می‌شوند
   - فایل‌ها کپی می‌شوند، پس حذف مدرک از پروندهٔ نماینده به مدارک درخواست دست نمی‌زند
   - فیلدهایی که ادمین قبلاً پر کرده بازنویسی نمی‌شوند

> ⚠️ **کاری که هنوز دستی است**: **سطح نمایندگی (tier)** خودکار تعیین نمی‌شود. بعد از تایید حتماً از `/admin/dealers/{id}/edit` سطح نمایندگی، آدرس و در صورت نیاز مشخصات دستگاه پوز را کامل کنید — بدون سطح، اجرت و سود نماینده صفر محاسبه می‌شود.
