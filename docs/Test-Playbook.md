...
25. [TS-NEG: تست‌های منفی (Negative Tests)](#25-ts-neg-تستهای-منفی)
...
| ID | اقدام | ورودی | نتیجه مورد انتظار | وضعیت |
|----|--------|-------|-------------------|--------|
| TS-22-08 | سفارش هدیه — شمش قبل از تحویل | U3 → سفارش هدیه + پرداخت → بررسی شمش | شمش SOLD + customer_id خالی + claim_code تولید‌شده + `delivered_at` خالی (NULL) | ☐ |
...
> این سند بر اساس قابلیت‌های فعلی سیستم TalaMala v4 (فازهای ۱-۱۴ + بازپرداخت اجرت بازخرید + بهبودهای تیکتینگ + صفحات ثابت و فوتر + ثبت مالکیت و انتقال شمش + ادغام Location در Dealer + لاگ درخواست‌ها + مدیریت کاربران + ویرایش مشتری توسط ادمین + انتخاب بسته‌بندی مشتری + گزارش فروش POS ادمین + مدیریت دارایی و قیمت‌گذاری خودکار + چند درگاه پرداخت + درخواست نمایندگی با بازبینی + پیگیری تحویل فیزیکی و طلای امانی + نظرات و پرسش‌وپاسخ محصول + API دستگاه POS مشتری‌محور) تهیه شده است.
