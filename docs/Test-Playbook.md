...
29. [TS-29: سیستم کنترل فعال/غیرفعال معاملات (Trade Toggle System)](#29-ts-29-سیستم-کنترل-فعالغیرفعال-معاملات-trade-toggle-system)
30. [TS-30: تولید QR Code شمش (QR Code Generation)](#30-ts-30-تولید-qr-code-شمش-qr-code-generation)
31. [TS-31: توزیع از مرکز پخش (Warehouse Distribution)](#31-ts-31-توزیع-از-مرکز-پخش-warehouse-distribution)
32. [TS-32: سیستم اعلان‌ها (Notifications)](#32-ts-32-سیستم-اعلانها-notifications)
33. [TS-33: پوشش ریسک — هجینگ (Hedging)](#33-ts-33-پوشش-ریسک--هجینگ-hedging)
34. [TS-NEG: تست‌های منفی (Negative Tests)](#34-ts-neg-تستهای-منفی)
...
## 29. TS-29: سیستم کنترل فعال/غیرفعال معاملات (Trade Toggle System)

| ID | اقدام | ورودی | نتیجه مورد انتظار | وضعیت |
|----|--------|-------|-------------------|--------|
| TG-01 | ماتریس تاگل معاملات در تنظیمات ادمین | لاگین ادمین → `/admin/settings` | ماتریس با ۲ ردیف (طلا/نقره) × ۷ ستون نمایش داده شود، تمام تاگل‌ها به صورت پیش‌فرض روشن (ON) باشند | ☐ |
| TG-02 | غیرفعال کردن خرید طلا از کیف پول → تلاش برای خرید | خاموش کردن `gold_wallet_buy` در تنظیمات → رفتن به `/wallet/gold` | فرم خرید غیرفعال باشد، پیام هشدار «خرید از کیف پول طلا در حال حاضر غیرفعال است» نمایش داده شود | ☐ |
| TG-03 | غیرفعال کردن فروش نقره از کیف پول → تلاش برای فروش | خاموش کردن `silver_wallet_sell` → رفتن به `/wallet/silver` | فرم فروش غیرفعال باشد، پیام هشدار نمایش داده شود | ☐ |
| TG-04 | غیرفعال کردن فروشگاه طلا → تلاش برای ثبت سفارش | خاموش کردن `gold_shop` → افزودن محصول طلایی به سبد → ثبت سفارش | ثبت سفارش با پیام خطا متوقف شود | ☐ |
| TG-05 | غیرفعال کردن POS نماینده طلا → فروش نماینده | خاموش کردن `gold_dealer_pos` → نماینده تلاش برای فروش POS شمش طلا | خطا: «فروش از پوز نماینده طلا در حال حاضر غیرفعال است» | ☐ |
| TG-06 | غیرفعال کردن POS مشتری طلا → رزرو از API | خاموش کردن `gold_customer_pos` → `POST /api/pos/reserve` برای شمش طلا | پاسخ JSON با خطا بازگردانده شود | ☐ |
| TG-07 | غیرفعال کردن سفارش عمده طلا → ثبت سفارش B2B | خاموش کردن `gold_b2b_order` → نماینده ثبت سفارش عمده با محصولات طلایی | خطا بازگردانده شود | ☐ |
| TG-08 | غیرفعال کردن بازخرید نقره → بازخرید شمش نقره | خاموش کردن `silver_buyback` → نماینده تلاش برای بازخرید شمش نقره | خطا بازگردانده شود | ☐ |
| TG-09 | فعال کردن مجدد تمام تاگل‌ها → بررسی عملکرد عادی | روشن کردن تمام ۱۴ تاگل | تمام عملیات خرید/فروش به صورت عادی کار کنند | ☐ |
| TG-10 | ماندگاری تاگل پس از ذخیره | خاموش کردن `gold_shop` → ذخیره تنظیمات → بارگذاری مجدد صفحه | تاگل `gold_shop` همچنان خاموش (OFF) باشد | ☐ |

---

## 30. TS-30: تولید QR Code شمش (QR Code Generation — On-the-fly)

> **توجه**: QR Code هرگز روی دیسک ذخیره نمی‌شود. هر درخواست دانلود، QR را در لحظه تولید و stream می‌کند. پوشه `qrcodes/` وجود ندارد.

| ID | اقدام | ورودی | نتیجه مورد انتظار | وضعیت |
|----|--------|-------|-------------------|--------|
| QR-01 | دانلود QR از پنل ادمین | صفحه ویرایش شمش → دانلود QR | فایل PNG با نام `QR_{SERIAL}.png` دانلود شود (تولید لحظه‌ای) | ☐ |
| QR-02 | بررسی ابعاد تصویر QR | باز کردن QR PNG دانلود شده | ابعاد تقریبی ~980x1060px، حالت RGB | ☐ |
| QR-03 | اسکن QR با گوشی | دوربین گوشی را به سمت QR بگیرید | آدرس `/verify/check?code=SERIAL` در مرورگر باز می‌شود | ☐ |
| QR-04 | بررسی لوگوی مرکزی QR | باز کردن تصویر QR | لوگوی برند (دایره با TM) در مرکز QR قابل مشاهده باشد | ☐ |
| QR-05 | بررسی متن سریال‌کد | باز کردن تصویر QR | متن سریال‌کد زیر QR Code چاپ شده باشد | ☐ |
| QR-06 | عدم وجود فایل روی دیسک | بررسی پوشه `static/uploads/` بعد از دانلود QR | هیچ پوشه `qrcodes/` یا فایل QR روی دیسک وجود نداشته باشد | ☐ |
| QR-07 | لوگوی سفارشی | قرار دادن `logo-qr.png` سفارشی در `static/assets/img/`، دانلود QR | QR از لوگوی سفارشی استفاده کند، نه لوگوی پیش‌فرض | ☐ |

---

## 31. TS-31: توزیع از مرکز پخش (Warehouse Distribution)

| ID | اقدام | ورودی | نتیجه مورد انتظار | وضعیت |
|----|--------|-------|-------------------|--------|
| WD-01 | مشاهده منو انتقال (مرکز پخش) | لاگین نماینده `is_warehouse=True` → سایدبار | آیتم «انتقال به نماینده» در منو نمایش داده شود | ☐ |
| WD-02 | عدم مشاهده منو (نماینده عادی) | لاگین نماینده عادی → سایدبار | آیتم «انتقال به نماینده» نمایش داده **نشود** | ☐ |
| WD-03 | دسترسی مستقیم توسط نماینده عادی | نماینده عادی → `/dealer/transfers` | خطای 403 | ☐ |
| WD-04 | انتقال موفق | مرکز پخش → انتخاب ۳ شمش + نماینده مقصد → ثبت | پیام موفقیت + ۳ شمش در موجودی مقصد منتقل شود + `dealer_id` تغییر کند | ☐ |
| WD-05 | انتقال بدون انتخاب شمش | مرکز پخش → بدون انتخاب شمش → ثبت | خطا: «هیچ شمشی انتخاب نشده است» | ☐ |
| WD-06 | تاریخچه انتقال | مرکز پخش → تب «تاریخچه» | لیست انتقالات ارسالی با نوع «توزیع مرکز پخش» نمایش داده شود | ☐ |
| WD-07 | فیلتر نوع فلز | مرکز پخش → فیلتر «طلا» | فقط شمش‌های طلایی نمایش داده شوند | ☐ |
| WD-08 | انتخاب همه | مرکز پخش → چک‌باکس «انتخاب همه» | تمام شمش‌ها انتخاب شوند + شمارنده بروزرسانی شود | ☐ |

---

## 32. TS-32: سیستم اعلان‌ها (Notifications)

### مرکز اعلان‌ها (مشتری)

| ID | اقدام | ورودی | نتیجه مورد انتظار | وضعیت |
|----|--------|-------|-------------------|--------|
| NTF-01 | مشاهده badge زنگوله | لاگین مشتری U3 → navbar | آیکون زنگوله نمایش داده شود | ☐ |
| NTF-02 | مشاهده لیست اعلان‌ها | U3 → `/notifications` | صفحه اعلان‌ها با صفحه‌بندی نمایش داده شود | ☐ |
| NTF-03 | خواندن تکی (AJAX) | U3 → کلیک روی اعلان خوانده‌نشده | اعلان خوانده شود + badge کاهش یابد | ☐ |
| NTF-04 | خواندن همه (AJAX) | U3 → دکمه «خواندن همه» | تمام اعلان‌ها خوانده شوند + badge صفر شود | ☐ |
| NTF-05 | polling شمارنده | U3 → ۶۰ ثانیه صبر | `GET /notifications/api/unread-count` فراخوانی شود | ☐ |

### تنظیمات اعلان

| ID | اقدام | ورودی | نتیجه مورد انتظار | وضعیت |
|----|--------|-------|-------------------|--------|
| NTF-06 | مشاهده تنظیمات | U3 → `/notifications/settings` | ماتریس ۱۶ نوع اعلان × ۲ کانال (SMS + In-app) نمایش داده شود | ☐ |
| NTF-07 | غیرفعال کردن SMS | U3 → غیرفعال SMS برای PAYMENT_SUCCESS → ذخیره | پرداخت بعدی فقط اعلان In-app ایجاد کند (بدون SMS) | ☐ |
| NTF-08 | غیرفعال کردن In-app | U3 → غیرفعال In-app برای WALLET_TRADE → ذخیره | خرید/فروش فلز اعلان In-app ایجاد نکند | ☐ |

### ارسال اعلان (ادمین)

| ID | اقدام | ورودی | نتیجه مورد انتظار | وضعیت |
|----|--------|-------|-------------------|--------|
| NTF-09 | ارسال به یک کاربر | ادمین → `/admin/notifications/send` → موبایل U3 + عنوان + متن | اعلان In-app برای U3 ایجاد شود | ☐ |
| NTF-10 | ارسال به همه مشتریان | ادمین → «همه مشتریان» + عنوان + متن | اعلان برای تمام کاربران غیر‌ادمین ایجاد شود | ☐ |
| NTF-11 | ارسال با SMS | ادمین → موبایل U3 + عنوان + متن + تیک SMS | اعلان In-app + SMS ارسال شود | ☐ |

### هوک‌های بیزینسی

| ID | اقدام | ورودی | نتیجه مورد انتظار | وضعیت |
|----|--------|-------|-------------------|--------|
| NTF-12 | پرداخت سفارش | U3 → پرداخت از کیف پول | اعلان PAYMENT_SUCCESS در `/notifications` + SMS | ☐ |
| NTF-13 | لغو سفارش | ادمین → لغو سفارش U3 | اعلان ORDER_STATUS با پیام لغو | ☐ |
| NTF-14 | تغییر وضعیت ارسال | ادمین → تغییر delivery_status سفارش | اعلان ORDER_DELIVERY | ☐ |
| NTF-15 | تأیید/رد برداشت | ادمین → تأیید/رد درخواست برداشت | اعلان WALLET_WITHDRAW + SMS | ☐ |
| NTF-16 | پاسخ تیکت | ادمین → پاسخ به تیکت مشتری | اعلان TICKET_UPDATE | ☐ |
| NTF-17 | انتقال مالکیت | U3 → انتقال شمش + تأیید OTP | اعلان OWNERSHIP_TRANSFER برای فرستنده و گیرنده | ☐ |

### badge در پنل‌های مختلف

| ID | اقدام | ورودی | نتیجه مورد انتظار | وضعیت |
|----|--------|-------|-------------------|--------|
| NTF-18 | badge ادمین | لاگین ادمین → sidebar | آیکون زنگوله + تعداد خوانده‌نشده | ☐ |
| NTF-19 | badge نماینده | لاگین نماینده → sidebar | آیکون زنگوله + تعداد خوانده‌نشده | ☐ |
| NTF-20 | badge مشتری | لاگین مشتری → navbar | آیکون زنگوله + تعداد خوانده‌نشده | ☐ |

---

## 33. TS-33: پوشش ریسک — هجینگ (Hedging)

### داشبورد

| ID | اقدام | ورودی | نتیجه مورد انتظار | وضعیت |
|----|--------|-------|-------------------|--------|
| HDG-01 | مشاهده داشبورد | ادمین → `/admin/hedging` | کارت‌های پوزیشن (طلا/نقره) + نمودار Chart.js + جدول آخرین تغییرات | ☐ |
| HDG-02 | رنگ‌بندی پوزیشن | بالانس منفی | کارت قرمز (Short) نمایش شود | ☐ |
| HDG-03 | رنگ‌بندی پوزیشن | بالانس مثبت | کارت سبز (Long) نمایش شود | ☐ |
| HDG-04 | رنگ‌بندی پوزیشن | بالانس صفر | کارت آبی (Hedged) نمایش شود | ☐ |
| HDG-05 | AJAX refresh | دکمه refresh در داشبورد | بالانس‌ها بدون رفرش صفحه بروزرسانی شوند | ☐ |

### ثبت معامله هج

| ID | اقدام | ورودی | نتیجه مورد انتظار | وضعیت |
|----|--------|-------|-------------------|--------|
| HDG-06 | ثبت خرید طلا (هج) | ادمین → `/admin/hedging/record` → طلا، خرید، ۱۰ گرم، قیمت | بالانس طلا +۱۰g شود + ردیف IN در دفتر کل | ☐ |
| HDG-07 | ثبت فروش نقره (هج) | ادمین → نقره، فروش، ۵۰ گرم، قیمت | بالانس نقره −۵۰g شود + ردیف OUT در دفتر کل | ☐ |
| HDG-08 | مقدار صفر یا منفی | ادمین → مقدار: ۰ | خطای اعتبارسنجی | ☐ |

### تعدیل بالانس

| ID | اقدام | ورودی | نتیجه مورد انتظار | وضعیت |
|----|--------|-------|-------------------|--------|
| HDG-09 | تنظیم بالانس اولیه | ادمین → `/admin/hedging/adjust` → طلا، ۱۰۰ گرم | بالانس طلا = ۱۰۰g + ردیف ADJUST در دفتر کل | ☐ |
| HDG-10 | تنظیم بالانس منفی | ادمین → طلا، −۵۰ گرم | بالانس طلا = −۵۰g (مجاز) | ☐ |

### دفتر کل

| ID | اقدام | ورودی | نتیجه مورد انتظار | وضعیت |
|----|--------|-------|-------------------|--------|
| HDG-11 | مشاهده دفتر کل | ادمین → `/admin/hedging/ledger` | لیست تمام تراکنش‌ها با صفحه‌بندی | ☐ |
| HDG-12 | فیلتر بر اساس فلز | فیلتر: طلا | فقط تراکنش‌های طلا نمایش شود | ☐ |
| HDG-13 | فیلتر بر اساس جهت | فیلتر: OUT | فقط تراکنش‌های خروجی نمایش شود | ☐ |
| HDG-14 | فیلتر بر اساس منبع | فیلتر: order | فقط تراکنش‌های سفارش فروشگاهی نمایش شود | ☐ |

### هوک‌های خودکار

| ID | اقدام | ورودی | نتیجه مورد انتظار | وضعیت |
|----|--------|-------|-------------------|--------|
| HDG-15 | سفارش فروشگاهی | مشتری → خرید شمش طلا از فروشگاه | ردیف OUT در دفتر کل هجینگ (وزن شمش) | ☐ |
| HDG-16 | فروش POS نماینده | نماینده → فروش POS | ردیف OUT در دفتر کل هجینگ | ☐ |
| HDG-17 | بازخرید | نماینده → بازخرید شمش | ردیف IN در دفتر کل هجینگ | ☐ |
| HDG-18 | خرید فلز کیف پول | مشتری → خرید طلا از کیف پول | ردیف OUT در دفتر کل هجینگ | ☐ |
| HDG-19 | فروش فلز کیف پول | مشتری → فروش طلا از کیف پول | ردیف IN در دفتر کل هجینگ | ☐ |
| HDG-20 | Idempotency | تراکنش تکراری (مثلاً retry) | ردیف مضاعف ایجاد نشود | ☐ |

### هشدار آستانه

| ID | اقدام | ورودی | نتیجه مورد انتظار | وضعیت |
|----|--------|-------|-------------------|--------|
| HDG-21 | هشدار فعال | بالانس از آستانه عبور کند | SMS هشدار به ادمین‌ها ارسال شود (daemon thread) | ☐ |
| HDG-22 | cooldown | دو تراکنش پشت سر هم از آستانه عبور کنند | فقط یک SMS ارسال شود (cooldown ۶۰ دقیقه) | ☐ |
| HDG-23 | هشدار غیرفعال | تنظیم `hedge_alert_enabled=false` | SMS ارسال نشود | ☐ |

---

## 34. TS-NEG: تست‌های منفی
