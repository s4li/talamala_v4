...
| TS-02-02 | فیلتر دسته‌بندی | کلیک روی یک دسته‌بندی | فقط محصولات آن دسته‌بندی نمایش شوند | ☐ |
...
| TS-08-04 | ایجاد محصول | `/admin/products/create` + فرم | محصول جدید ایجاد شود | ☐ |
| TS-08-05 | ویرایش محصول | `/admin/products/{id}/edit` + فرم | تغییرات ذخیره شود | ☐ |
...
| # | شرح تست | پیش‌شرط | اقدام | نتیجه مورد انتظار |
|---|---------|---------|-------|-------------------|
| TS-13-05 | دریافت لیست محصولات | نماینده با شمش‌های فعال | GET /api/dealer/products | 200 + لیست محصولات با قیمت و سریال |
...
**پیش‌نیاز**: لاگین مشتری + شمش با claim_code و بدون customer_id

| ID | اقدام | ورودی | نتیجه مورد انتظار | وضعیت |
|----|--------|-------|-------------------|--------|
| TS-16-08 | باز کردن صفحه ثبت مالکیت | لاگین U4 → `/claim-bar` | فرم با فیلدهای سریال‌کد و کد ثبت مالکیت نمایش شود | ☐ |
| TS-16-09 | ثبت مالکیت موفق | سریال‌کد شمش POS فروخته‌شده + claim_code صحیح → «ثبت مالکیت» | مالکیت ثبت شود + customer_id به U4 تغییر کند + claim_code حذف شود + پیام موفقیت | ☐ |
| TS-16-10 | ثبت مالکیت — claim_code اشتباه | سریال‌کد معتبر + claim_code اشتباه | خطا: کد ثبت مالکیت نامعتبر | ☐ |
| TS-16-11 | ثبت مالکیت — سریال‌کد نامعتبر | سریال‌کد ناموجود + هر claim_code | خطا: شمش یافت نشد | ☐ |
| TS-16-12 | ثبت مالکیت — شمش قبلاً ثبت شده | سریال‌کد شمشی که customer_id دارد | خطا: شمش قبلاً به مالکی ثبت شده | ☐ |
| TS-16-13 | ثبت مالکیت بدون لاگین | بدون لاگین → `/claim-bar` | ریدایرکت به صفحه ورود | ☐ |

### مشاهده شمش‌های من (My Bars)

**پیش‌نیاز**: لاگین مشتری با حداقل یک شمش ثبت‌شده

| ID | اقدام | ورودی | نتیجه مورد انتظار | وضعیت |
|----|--------|-------|-------------------|--------|
| TS-16-14 | مشاهده لیست شمش‌ها | لاگین U3 → `/my-bars` | لیست شمش‌های متعلق به مشتری با سریال‌کد، نام محصول، وزن، عیار | ☐ |
| TS-16-15 | مشتری بدون شمش | لاگین U5 (بدون شمش) → `/my-bars` | پیام «هیچ شمشی ثبت نشده» یا لیست خالی | ☐ |
| TS-16-16 | دکمه انتقال | `/my-bars` → دکمه «انتقال» کنار هر شمش | لینک به صفحه انتقال `/my-bars/{bar_id}/transfer` | ☐ |

### انتقال مالکیت (Ownership Transfer)

**پیش‌نیاز**: لاگین U3 + حداقل یک شمش ثبت‌شده به نام U3

| ID | اقدام | ورودی | نتیجه مورد انتظار | وضعیت |
|----|--------|-------|-------------------|--------|
| TS-16-17 | باز کردن فرم انتقال | لاگین U3 → `/my-bars/{bar_id}/transfer` | فرم با فیلد شماره موبایل گیرنده نمایش شود | ☐ |
| TS-16-18 | ارسال OTP انتقال | وارد کردن موبایل گیرنده (مثلاً U4) → «ارسال کد تایید» | OTP به موبایل مالک (U3) ارسال شود + فرم وارد کردن OTP نمایش شود | ☐ |
| TS-16-19 | تایید انتقال موفق | OTP صحیح (111111 در حالت dev) → «تایید انتقال» | مالکیت منتقل شود + customer_id شمش به U4 تغییر کند + OwnershipHistory ثبت شود + پیام موفقیت | ☐ |
| TS-16-20 | تایید انتقال — OTP اشتباه | OTP نادرست → «تایید انتقال» | خطا: کد تایید نادرست | ☐ |
| TS-16-21 | انتقال به موبایل جدید (بدون حساب) | موبایل گیرنده‌ای که حساب ندارد → OTP صحیح → تایید | حساب مشتری جدید ایجاد شود + مالکیت منتقل شود | ☐ |
| TS-16-22 | انتقال شمش دیگری | لاگین U4 → `/my-bars/{bar_id}/transfer` (شمش متعلق به U3) | خطای 403 / عدم دسترسی | ☐ |
| TS-16-23 | شمش پس از انتقال در my-bars گیرنده | لاگین U4 → `/my-bars` (پس از دریافت شمش از U3) | شمش منتقل‌شده در لیست U4 نمایش شود | ☐ |
| TS-16-24 | شمش پس از انتقال از my-bars فرستنده حذف شود | لاگین U3 → `/my-bars` (پس از انتقال) | شمش منتقل‌شده دیگر در لیست U3 نباشد | ☐ |
