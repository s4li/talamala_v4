...
31. [TS-NEG: تست‌های منفی (Negative Tests)](#31-ts-neg-تستهای-منفی)
...
## 30. TS-30: تولید QR Code شمش (QR Code Generation)

| ID | اقدام | ورودی | نتیجه مورد انتظار | وضعیت |
|----|--------|-------|-------------------|--------|
| QR-01 | تولید شمش از پنل ادمین | تعداد: ۵ | ۵ شمش ایجاد شده + ۵ فایل QR PNG در `static/uploads/qrcodes/` | ☐ |
| QR-02 | بررسی ابعاد تصویر QR | باز کردن هر QR PNG تولید شده | ابعاد تقریبی ~980x1060px، حالت RGB | ☐ |
...
| QR-06 | دانلود QR از پنل ادمین | صفحه ویرایش شمش → دانلود QR | فایل PNG با نام `QR_{SERIAL}.png` دانلود شود | ☐ |
| QR-07 | بازسازی QR | صفحه ویرایش شمش → بازسازی | QR قدیمی جایگزین شده، صفحه به ویرایش بازگردد | ☐ |
| QR-08 | QR برای فایل حذف‌شده | حذف دستی فایل QR، سپس مراجعه به صفحه ویرایش شمش | QR به صورت خودکار (on-demand) تولید و نمایش داده شود | ☐ |
| QR-09 | تولید QR توسط اسکریپت seed | اجرای `seed.py --reset` | تمام شمش‌ها QR Code دریافت کنند، تعداد در خروجی چاپ شود | ☐ |
| QR-10 | لوگوی سفارشی | قرار دادن `logo-qr.png` سفارشی در `static/assets/img/`، بازسازی QR | QR از لوگوی سفارشی استفاده کند، نه لوگوی پیش‌فرض | ☐ |
