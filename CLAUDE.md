...
- `GET /admin/bars?tier_id=X` — Filter bar list by dealer tier (سطح نمایندگی) of the bar's physical location; combines with `dealer_id`/`status`/`product_id`/`sellable`
