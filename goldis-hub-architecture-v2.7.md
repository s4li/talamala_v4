...
1. **متن اصلی سند قطعی و تمیز است** — مستقیماً مدلِ نهایی را می‌گوید (بدون متن خط‌خورده/منسوخ). آن را معتبر بخوان.
2. **§۲.۵ «دفتر تصمیمات» (D-46…D-80) غیرقطعی است** — فقط *چراییِ* تغییرات برای حسابرسی. برای ساختِ سیستم به بدنه نگاه کن، نه این جدول.
3. همه‌ی `Q-01…Q-10` و نقاطِ `A-1…A-13` / `F-1…F-4` **حل شده‌اند** (نگاشتشان در §۲۰/§۲.۵).
4. **تمرکز بازبینی روی §۰.۱ (موارد باز) باشد** — تصمیماتِ §۲.۵ با تیم نهایی شده‌اند؛ بازنکن مگر یک **باگِ مالی/همزمانی/تفکیک‌وظایفِ اثبات‌پذیر** نشان دهی.
...
5. ابهامات: همه‌ی Q-01…Q-10 در §۲۰ حل شده‌اند. موارد بازِ باقی‌مانده در **§۰.۱** فهرست شده — قبل از پیاده‌سازیِ context مربوطه باید حل شوند.
...
شمش فیزیکی، طلای دیجیتال، marketplace چندکاناله، wallet چنددارایی per-wallet-scope (D-46: goldis/aminzar/talamala ایزوله)، DealerNetwork، Treasury مرکزی، **Inter-Company Hedging Ledger** (real-time hub-and-spoke با Goldis در مرکز)، Accounting، Fulfillment عملیاتی، Realtime، Notification، Audit کامل، Pricing مرکزی با snapshot، **rial withdrawal** دومرحله‌ای، physical_purchase_from_wallet (به‌جای gold withdrawal)، **Buyback: undelivered + physical (D-58/D-68)**.
...
| D-71 | **حل A-8 — برچسبِ scope روی شمش (`sale_wallet_scope`)** | `bars` ستونِ `sale_wallet_scope VARCHAR(20) NULL` می‌گیرد: در لحظه‌ی فروش از scopeِ سفارش (goldis/aminzar/talamala) پر و IMMUTABLE **تا زمان status=SOLD** (انتقالِ مالکیت — D-55 — عوضش نمی‌کند، چون حساب به فروشِ اول گره خورده). هنگام بازخرید (status برمی‌گردد به ASSIGNED)، `sale_wallet_scope=NULL` می‌شود تا شمش دوباره در هر scope فروخته شود (D-92). همه‌ی مسیرهای بازخرید/انتقال/گزارشِ تفکیکیِ D-46 از این برچسب تصمیم می‌گیرند، نه استنتاجِ ضمنی. شمشِ فروش‌نرفته=`NULL` (مالکیتِ شرکتی). برای کارت‌هدیه/مارکت‌پلیس (بدون ثبت‌مالکیت — D-54): `sale_wallet_scope` پر می‌شود (برای خزانه/بین‌شرکتی) ولی `customer_id=NULL` — دو بُعدِ مستقل. **قاعده:** بازخریدِ آنلاین فقط در همان scope/وب‌سایتی که خرید انجام شده مجاز است. | Inventory، Wallet، Buyback، §۱۱.۵ |
...
| D-90 | **علامت delta در hedge_buy** | در flowِ `/admin/treasury/hedge-buy/request`، `Treasury.record(source=hedge_buy, delta)` باید **منفی** باشد (−amount_mg)، نه مثبت. دلیل: hedge_buy exposure را **کاهش** می‌دهد (خزانه را تقویت می‌کند). علامت مثبت باعث می‌شود exposure دوبرابر شود و فروش‌های جدید قفل شوند. | Treasury، §۱۲.۵.۳ الف |
| D-91 | **pure_gold_mg در تمام Treasury.record calls** | در فروش‌های فیزیکی (§۱۲.۱/POS/Marketplace/فیزیکی از wallet)، `Treasury.record(delta=+weight_mg)` نادرست است. صحیح: `delta=+pure_gold_mg = weight × purity / 1000`. دلیل: hedging مبنای exposure روی `pure_gold_mg` است، نه وزن خام. عدم رعایت این قاعده باعث عدم تطابق exposure با تعهدات واقعی و ریسک مالی شدید می‌شود. | Treasury، §۱۲.۱، §۱۲.۷، §۱۲.۸ |
| D-92 | **sale_wallet_scope نباید برای همیشه IMMUTABLE باشد** | طبق D-71، `sale_wallet_scope` IMMUTABLE تعریف شده بود. اما وقتی شمش به وسیلهِ بازخرید برمی‌گردد به تالار (status=ASSIGNED, customer_id=NULL)، این فیلد باید NULL شود تا شمش در هر scope دوباره فروخته شود. بدون این، cross-scope resale ممکن نیست و contradiction ایجاد می‌شود. | Inventory، Buyback، D-71 |
| D-93 | **API برای D-62 (two-stage inventory transfer)** | جداول `inventory_transfer_documents` و `items` وجود دارند اما هیچ endpoint برای create/dispatch/receive/complete معرفی نشده. بدون این API، انبارداران نمی‌توانند از فرایند استفاده کنند. ✅ **اصلاح v2.7**: 8 endpoint در §۱۳ اضافه شد: `POST /admin/inventory/transfers`, `GET .../transfers`, `GET .../transfers/{id}`, `POST .../dispatch`, `POST .../receive`, `POST .../confirm`, `POST .../discrepancy`, `POST .../cancel`. | Inventory، §۱۲، §۱۳ |
| D-94 | **جدول dealer_sales گمشده** | `dealer_commission_ledger` FK به `dealer_sale_id` دارد اما این جدول تعریف نشده است. لازم: جدولِ `dealer_sales` با فیلدهای اصلی (id, dealer_user_id, order_id, bar_id, pure_gold_mg, sale_type). | Dealer، Schema |
| D-95 | **Hedge Buy و Dealer Commission Offset در Roadmap نیستند** | فازهای §۲۱ پیاده‌سازی هیچ اشاره‌ای به این دو قابلیتِ حیاتی ندارند. Hedge Buy برای Treasury ضروری و Offset برای جلوگیری از نشت مالی لازم است. لازم: Hedge Buy در فاز ۳ و Commission Offset در فاز ۴. | Roadmap، D-82، D-84 |

> **⚠️ اثر روی scope/تخمین:** D-47، D-48 و D-53 scope را سنگین‌تر کرده‌اند (supplier_purchase داخل؛ سقف دوطرفه؛ گیت ثبت‌مالکیت). D-58/D-59 و حذفِ SubDealer/MLM (D-73) برعکس **ساده‌تر** کرده‌اند. **هشدارِ P۵:** زیرسیستمِ **قیمت(D-65/72) + دفترکل(§۶) + خزانه(D-47/67) + نماینده(D-73)** الان سنگین‌ترین و پرریسک‌ترین بخش پروژه است (باگ = ضررِ مالیِ مستقیم). توصیه: در پیاده‌سازی **اول یک پروتوتایپِ end-to-end از همین زنجیره با تست‌های پولیِ واقعی** ساخته شود، نه CRUDهای ساده. تخمین ۱۲ هفته‌ی بخش ۲۲ دیگر واقع‌بینانه نیست و باید بازبینی شود.
...
| D-96 | **Price Lock Reconciliation** — قفل قیمت ۵دقیقه‌ای | بین checkout و پرداخت اگر قیمت تغییر کند (شبکه/تأخیر)، سیستم پول را تو خلأ نندازد. **راه‌حل:** `payment_reconciliations` جدول → تطابق خودکار ±۲٪، treasury rebalancing، manual dashboard برای admin. |
| D-97 | **Pending Reserves** — رزرو طلا بر روی checkout | سقف خزانه (treasury exposure cap) در checkout checkedشود، سپس در submit order رزرو شود. دو customer هم‌زمان هم نتوانند سقف را نقض کنند. **راه‌حل:** `inventory_pending_holds` جدول → reserve on checkout، finalize on payment. |
| D-98 | **Payment State Machine** — منع پول یتیم | اگر سیستم بعد از gateway verification crash کند، دوباره مدار شود. **راه‌حل:** `payment_states` enum = `verified_pending` (state جدید) → recovery job on startup replays. idempotency + alerts. |
| D-99 | **POS Offline Queue** — دستگاه‌های بدون اینترنت | اگر WiFi قطع شود، `/api/pos/confirm` موفق نشود. **راه‌حل:** Local SQLite queue on POS device → `payment_pending_queue` جدول on server → retry with `request_id` idempotency. |
...
| 10 | **wallet** | Per-wallet-scope | AssetBalance، LedgerEntry، Lock |
...
CREATE TABLE dealer_sales (
    ...
    bar_id BIGINT NULL REFERENCES bars(id),                 -- شمشِ فروخته‌شده (اختیاری — برای سفارش‌های دیجیتالی)
...
8. on verified:
   - Wallet scope=aminzar, company=Goldis Co. (XAU_MG) of user → +500mg  # D-46: ایزوله، نه merge در Goldis
...
12. confirm (after gateway verified یا اگر gateway نداشت بلافاصله):
    ...
    - Treasury.record دو پای مستقل (D-66 — آینهٔ D-59):
        ...
        پای۲: delta=+pure_gold_mg            # خروجِ فقطِ فلزِ خالصِ شمشِ تحویلی ⇒ exposure باز (اجرت منهی)
      ...
      ⚠️ ثبتِ تنها پای۱ (متن قدیمی) باگ است: یک شمشِ هج‌نشده از سیستم خارج و exposure گم می‌شود
...
> **موارد بازِ باقی‌مانده در §۰.۱** فهرست شده‌اند (تمرکز بازبینی بعدی آنجاست).
...
12. Wallet (per-wallet-scope multi-asset ledger — D-46: goldis/aminzar/talamala ایزوله)
...
23. **Hedge Buy flow + bulk_gold_inventory intake** (D-95): Goldis خریدِ طلایِ خام از بازار/ارزی یا دریافتِ طلا از تولیدکننده‌ها (supplier purchase D-48)؛ ثبتِ در treasury (−exposure)؛ تولیدِ شمش و توزیع در شبکه.
...
CREATE TABLE pos_pending_requests (
    ...
    request_id VARCHAR(100) NOT NULL,                 -- uniqueness per device
    ...
    UNIQUE (dealer_id, pos_session_id, request_id)   -- idempotency per device
);
